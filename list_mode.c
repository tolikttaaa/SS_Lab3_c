...
char *run_list_mode() {
    ...
    for (int i = 0; i < disks_counter; i++) {
        res = concat(res, "disk name: ");
        res = concat(res, disks[i].disk_name);
        res = concat(res, "\n");
    }

    for (int i = 0; i < parts_counter; i++) {
        res = concat(res, "partition name: ");
        res = concat(res, partitions[i].disk_name);
        res = concat(res, " (");
        res = concat(res, partitions[i].fs_type);
        res = concat(res, " ");
        res = concat(res, partitions[i].fs_version);
        res = concat(res, ")\n");
    }
